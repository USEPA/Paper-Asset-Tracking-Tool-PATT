...
"SELECT a.id, a.lan_id, sum(a.box_destroyed) as box_destroyed, sum(b.freeze) as freeze, c.name as box_status, a.box_status as box_status_id, a.box_id, d.ticket_priority as ticket_priority, e.name  as priority_name
...
INNER JOIN wpqa_terms e ON d.ticket_priority = e.term_id
...
<table id="tbl_templates_box_details" class="display nowrap" cellspacing="5" cellpadding="5">
        <thead>
            <tr>
                    <?php		
                    if (($agent_permissions['label'] == 'Administrator') || ($agent_permissions['label'] == 'Agent') || ($agent_permissions['label'] == 'Manager'))
...
if(!empty($lan_id_details) && ($lan_id_details != 'Error') && ($lan_id_details != 'LAN ID cannot be assigned') && (strtoupper($lan_id_username) == strtoupper($obj->{'lan_id'})) ) {
