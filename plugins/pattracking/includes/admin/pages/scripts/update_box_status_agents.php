...
// 			print_r($status_obj);
			...
			if( $user_str == '' ) {
				//$status_and_users_str .= $status_str.': '.$user_str.' | ';
			} else {
// 				$status_and_users_str .= $status_str.': '.$user_str.'  |  ';
		...
		//$status_and_users_str = substr($status_and_users_str, 0, -5);
		$status_and_users_str = '<ul>'.$status_and_users_str.'</ul>';
		
		//$status_str = get_term_by( 'id', Patt_Custom_Func::get_box_file_details_by_id($id)->box_status, 'wpsc_box_statuses');		
...
/*
		...
		echo '<br>status string: '.$status_and_users_str;
*/
	}
