...
// if view message
	if ( isset( $_GET['action'] ) && 'view' == $_GET['action'] && !empty( $_GET['id'] ) )
	{
		...
		$wpdb->update( $wpdb->prefix . 'pm_users', array( 'viewed' => 1 ), array( 'pm_id' => $id, 'recipient' => $current_user->ID ) );

		// select message information
		$msg = $wpdb->get_row( 'SELECT a.id, a.subject, a.content, b.viewed, b.deleted, a.date FROM ' . $wpdb->prefix . 'pm a INNER JOIN ' . $wpdb->prefix . 'pm_users b ON a.id = b.pm_id WHERE a.id = "' . $id . '" LIMIT 1' );
	...
	<div class="wrap">
		...
		<table class="widefat fixed" cellspacing="0">
			...
			<tbody>
			<tr>
				<td><?php printf( __( '<strong>Date</strong>: %s', 'pm4wp' ), $msg->date ); ?></td>
				<td><?php printf( __( '<p><strong>Subject</strong>: %s</p><p>%s</p>', 'pm4wp' ), stripcslashes( $msg->subject ) , nl2br( stripcslashes( $msg->content ) ) ); ?></td>
				<td>
						<span class="delete">
							<a class="delete"
...
// don't need to do more!
	...
	if ( isset( $_GET['action'] ) && 'mar' == $_GET['action'] && !empty( $_GET['id'] ) )
	{
		...
		$check_read = $wpdb->get_row( 'SELECT viewed FROM ' . $wpdb->prefix . 'pm_users WHERE pm_id = "' . $_GET['id'] . '" AND recipient = "' . $current_user->ID . '"' );

		if ($check_read->viewed != 1){
		if ( $wpdb->query( 'UPDATE ' . $wpdb->prefix . 'pm_users SET `viewed` = "1" WHERE `recipient` = "' . $current_user->ID . '" AND `pm_id` IN (' . $id . ')' ) )
		{
	...
	if ( isset( $_GET['action'] ) && 'delete' == $_GET['action'] && !empty( $_GET['id'] ) )
	{
		...
		foreach ( $id as $msg_id )
		{
			...
			$sender_deleted = $wpdb->get_var( 'SELECT `deleted` FROM ' . $wpdb->prefix . 'pm_users WHERE `pm_id` = "' . $msg_id . '" AND `recipient` = "' . $current_user->ID . '" LIMIT 1' );

			// create corresponding query for deleting message
			if ( $sender_deleted == 1 )
			{
				//$query = 'DELETE from ' . $wpdb->prefix . 'pm WHERE `id` = "' . $msg_id . '"';
				$query = 'UPDATE ' . $wpdb->prefix . 'pm_users SET `deleted` = "2" WHERE `pm_id` = "' . $msg_id . '" AND `recipient` = "' . $current_user->ID . '"';
	...
	$msgs = $wpdb->get_results( 'SELECT a.id, a.subject, b.viewed, a.date FROM ' . $wpdb->prefix . 'pm a INNER JOIN ' . $wpdb->prefix . 'pm_users b ON a.id = b.pm_id WHERE b.recipient = "' . $current_user->ID . '" AND b.deleted != "2" ORDER BY a.date DESC' );
	?>
<div class="wrap">
